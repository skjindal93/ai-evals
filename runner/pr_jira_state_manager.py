...
1. resolve a Jira issue from the PR title (via Atlassian MCP search), and
...
class AtlassianWorkflowClient(Protocol):
    ...
    Implementations should call Atlassian MCP primitives to:
    - locate the Jira issue from a PR title, and
    - transition that issue into the "Under Review" state.
    """
